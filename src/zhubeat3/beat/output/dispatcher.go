...
func (d *Dispatcher) init(clientArgs *ClientArgs) error {
	...
	newclientFunc := func() (pool.PoolClient, error) {
		conn, err := transport.NewClient(clientArgs.network, clientArgs.addresses, clientArgs.timeout)
		if err != nil {
			return nil, err
		}
		client := NewOutputClient(conn, clientArgs.ttl)
		err = client.RandomConnect()
		return client, err
	}
	...
	for {
		...
		case <-d.ticker.C:
			...
			if jobBatch.Len() > 0 {
				...
				worker := d.workerPool.TakeClient().(*OutputClient)
				go func(worker *OutputClient, jobBatch job.BatchJob) {
