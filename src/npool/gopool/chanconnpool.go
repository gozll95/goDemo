...
func (pool *ChanConnPool) getConnCount(addr string) int32 {
	pool.alocker.RLock()
	defer pool.alocker.RUnlock()
	if attr, ok := pool.addrs[addr]; ok {
...
func (pool *ChanConnPool) getRAddr(addrs map[string]*addrAttr) (string, error) {
	...
	if host_len == 0 {
		...
		if len(pool.al) != 0 {
			...
			return pool.GetMostConnAddr(), nil
		}
	}
...
func (pool *ChanConnPool) checkOnce() {
	for addr := range pool.getAllAddr() {
		//fmt.Printf("pool.getAddrHealth(%v):%v", addr, pool.getAddrHealth(addr))
		if !pool.getAddrHealth(addr) {
			if time.Now().Unix()-pool.getCooldownTime(addr) > pool.maxCooldownTime {
				pool.resetAddr(addr)
			}
		} else {
			//fmt.Printf("pool.getAddrErrCount(%v):%v\n", addr, pool.getAddrErrCount(addr))
...
func (pool *ChanConnPool) Get() (interface{}, error) {
	// defer pool.getOrPutLocker.Unlock()
	// pool.getOrPutLocker.Lock()
...
func (pool *ChanConnPool) GetMostConnAddr() string {
	// fmt.Println("xxx")
	// var mostConn int32
	// var moseConnAddr string

	// mostConn = pool.addrs[pool.al[0]].conncount
	// for _, al := range pool.al {
	// 	if pool.addrs[al].health {
	// 		conn := pool.addrs[al].conncount
	// 		if conn >= mostConn {
	// 			mostConn = conn
	// 			moseConnAddr = al
	// 		}
	...
	// fmt.Println("least conn is ", mostConn)
	// fmt.Println("least addr is ", moseConnAddr)
	// return moseConnAddr

	pool.mlocker.RLock()
	defer pool.mlocker.RUnlock()

	var attrBorrow map[string]int
	attrBorrow = make(map[string]int)

	for _, connAttr := range pool.mapConn {
		if connAttr.borrow == true {
			attrBorrow[connAttr.addr]++
		}
	}

	var least int
	var leastAttr string
	for attr, borrow := range attrBorrow {
		if borrow <= least {
			least = borrow
			leastAttr = attr
		}
	...
	if least == 0 {
		return pool.al[pool.r.Intn(len(pool.al))]
	}

	fmt.Println("least conn is ", least)
	fmt.Println("least addr is ", leastAttr)
	return leastAttr
